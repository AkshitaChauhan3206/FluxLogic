...
def generate_response(query, dataset_path, secret_key):
    try:
        if not os.path.exists(dataset_path):
            return "🕵️ I couldn't find your dataset file on the server. It might have been removed or lost during a session reset. Please try re-uploading it! 😊"
        df = decrypt_file_to_df(dataset_path, secret_key)
    except Exception as e:
        print(f"Decryption error: {e}")
        return "❌ I encountered an encryption mismatch. This usually happens if the server's security keys were updated. Please re-upload your dataset to fix this! 😊"
    ...
    if not forced_intent and confidence < 0.08:
        ...
        if found_col:
            if found_col in numeric_cols:
                return f"📊 For **{found_col}**, the total is {fmt(df[found_col].sum(), currency)} with an average of {fmt(df[found_col].mean(), currency)}. {outro}"
            else:
    ...
    if best_intent == "summary":
        ...
        if val_col: res.append(f"• **Value**: Your total accumulated **{val_col}** stands at **{fmt(df[val_col].sum(), currency)}**. That's quite impressive! ✨")
    ...
    if best_intent == "profit" and val_col:
        ...
        avg = df[val_col].mean()
        return f"{intro} The total **{val_col}** calculated from your data is **{fmt(total, currency)}**. On average, each record contributes about {fmt(avg, currency)} to your business goals. {outro}"
    ...
    if best_intent == "top_items" and cat_col and val_col:
        ...
        for k, v in top.items():
            res.append(f"• **{k}**: Contributing **{fmt(v, currency)}** in total value. 🏆")
        return "\n".join(res) + outro
    ...
    if best_intent == "worst_items" and cat_col and val_col:
        ...
        for k, v in worst.items():
            res.append(f"• **{k}**: Total value of **{fmt(v, currency)}**. 📉")
        return "\n".join(res) + outro

    # ── AVERAGE ──
    if best_intent == "average" and val_col:
        avg_val = df[val_col].mean()
        return f"{intro} The average **{val_col}** across your dataset is **{fmt(avg_val, currency)}**. This is your baseline performance metric. {outro}"
    ...
    if best_intent == "trend" and date_col and val_col:
        ...
        for k, v in recent.items():
            res.append(f"• On {k}: You recorded **{fmt(v, currency)}**. 📈")
...
def analyze_dataset(dataset_path, secret_key):
    ...
    summary = f"I have analyzed {num_rows} records. Total {val_col} is {fmt(df[val_col].sum(), currency)}." if val_col else f"Analyzed {num_rows} records."
    ...
    for c in numeric_cols[:6]:
        ...
        numeric_stats.append({
            "column": c.capitalize(),
            "total": fmt(col_data.sum(), currency), "average": fmt(col_data.mean(), currency),
            "min": fmt(col_data.min(), currency), "max": fmt(col_data.max(), currency), "std": fmt(col_data.std(), currency)
        })

    return {
        "summary": summary, "numeric_stats": numeric_stats, "patterns": patterns, "pareto": pareto_insight,
        "top_performers": [{"name": str(k), "value": fmt(v, currency)} for k, v in df.groupby(cat_col)[val_col].sum().sort_values(ascending=False).head(5).items()] if cat_col and val_col else [],
        "weak_performers": [{"name": str(k), "value": fmt(v, currency)} for k, v in df.groupby(cat_col)[val_col].sum().sort_values(ascending=True).head(3).items()] if cat_col and val_col else [],
